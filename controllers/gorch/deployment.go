...
func (r *GuardrailsOrchestratorReconciler) createDeployment(ctx context.Context, orchestrator *gorchv1alpha1.GuardrailsOrchestrator) *appsv1.Deployment {
	...
	if orchestrator.Spec.EnableDetectors {
		...
		log.FromContext(ctx).Info("Using regex detectors image " + detectorImage + " " + "from configmap " + r.Namespace + ":" + constants.ConfigMap)
		containerImages.DetectorImage = detectorImage
	}
	// Check if the vLLM gateway is enabled
	if orchestrator.Spec.EnableVLLMGateway {
		vllmGatewayImage, err := r.getImageFromConfigMap(ctx, vllmGatewayImageKey, constants.ConfigMap, r.Namespace)
		if vllmGatewayImage == "" || err != nil {
			log.FromContext(ctx).Error(err, "Error getting vLLM gateway image from ConfigMap.")
		}
		log.FromContext(ctx).Info("Using vLLM gateway image " + vllmGatewayImage + " " + "from configmap " + r.Namespace + ":" + constants.ConfigMap)

		containerImages.VLLMGatewayImage = vllmGatewayImage
	}
