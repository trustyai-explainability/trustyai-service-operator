...
func (r *NemoGuardrailsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if nemoGuardrails.Spec.ExposeRoute == nil {
		...
		patch := client.MergeFrom(nemoGuardrails.DeepCopy())
		nemoGuardrails.Spec.ExposeRoute = &exposeRouteTrue
		if err := r.Patch(ctx, nemoGuardrails, patch); err != nil {
			utils.LogErrorUpdating(ctx, err, "NemoGuardrails spec", nemoGuardrails.Name, nemoGuardrails.Namespace)
			return ctrl.Result{}, err
		}
