...
var _ = Describe("buildDeploymentSpec", func() {
	...
	It("builds the expected DeploymentSpec (labels, eval-hub, kube-rbac-proxy, strategy)", func() {
		...
		Expect(container.StartupProbe.HTTPGet.Port).To(Equal(intstr.FromInt(metricsPort)))
		...
		Expect(container.ReadinessProbe.HTTPGet.Port).To(Equal(intstr.FromInt(metricsPort)))
