...
var _ = Describe("buildDeploymentSpec", func() {
	...
	It("builds the expected DeploymentSpec (labels, eval-hub, kube-rbac-proxy, strategy)", func() {
		...
		Expect(container.StartupProbe.HTTPGet.Path).To(Equal(evalHubInternalHealthPath))
		Expect(container.StartupProbe.HTTPGet.Port).To(Equal(intstr.FromInt(servicePort)))
		Expect(container.StartupProbe.HTTPGet.Scheme).To(Equal(corev1.URISchemeHTTPS))
		Expect(container.ReadinessProbe).NotTo(BeNil())
		Expect(container.ReadinessProbe.HTTPGet.Path).To(Equal(evalHubInternalHealthPath))
		Expect(container.ReadinessProbe.HTTPGet.Port).To(Equal(intstr.FromInt(servicePort)))
		...
		Expect(krp.ReadinessProbe).To(BeNil())
