...
func (r *NemoGuardrailsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	_, err = utils.ReconcileConfigMap(ctx, r.Client, nemoGuardrails, nemoGuardrails.Name+"-ca-bundle", caBundleTemplate, templateParser.ParseResource)
	if err != nil {
	...
	if err := r.Status().Update(ctx, nemoGuardrails); err != nil {
		logger.Error(err, "Failed to update CA status")
		return ctrl.Result{}, err
	}
