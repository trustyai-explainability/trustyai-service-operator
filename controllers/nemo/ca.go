...
func (r *NemoGuardrailsReconciler) LoadCAConfigs(ctx context.Context, logger logr.Logger, nemoGuardrails nemov1alpha1.NemoGuardrails) (utils.CABundleInitContainerConfig, []corev1.ConfigMap, *nemov1alpha1.CAStatus) {
	// === CA handling =====
	...
	caStatus := &nemov1alpha1.CAStatus{}
	...
	if nemoGuardrails.Spec.CABundleConfig != nil {
		...
		if err != nil {
			logger.Error(err, "Could not find or load the user-specified CA bundle configmap, so will not be mounted.")
